...
class VearchIndex(Vearch):
    def __init__(self, metric, nlist, noise_shaping_threshold, num_dims_per_block):
        self.nlist = nlist
        if metric == "euclidean":
            self.metric = "L2"
        else:
            self.metric = "InnerProduct"
        self.noise_shaping_threshold = noise_shaping_threshold
        self.num_dims_per_block = num_dims_per_block
    
    def __str__(self):
        return "VearchIndex(nlist=%d, num_dims_per_block=%d, nprobe=%d, rerank=%d)" % (self.nlist, self.num_dims_per_block, self.nprobe, self.rerank)
    ...
    def fit(self, X):
        ...
        d = X.shape[1]
        self.nsubvector = int(d / self.num_dims_per_block)
        self.engine = vearch.Engine("files", "logs")
        table = {
            "name" : "test_table",
            "engine" : {
                "index_size": X.shape[0],
                "retrieval_type": "SCANN",       
                "retrieval_param": {
                    "metric_type": self.metric,
                    "ncentroids": self.nlist,
                    "nsubvector": self.nsubvector,
                    "scann_reordering": True,
                    "noise_shaping_threshold": self.noise_shaping_threshold
                }
            },
            "properties" : {
                "feature": {
    ...
    def set_query_arguments(self, nprobe_rerank):
        self.nprobe, self.rerank = nprobe_rerank
