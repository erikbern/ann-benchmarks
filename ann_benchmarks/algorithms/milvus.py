...
class Milvus(BaseANN):
    ...
    def fit(self, X):
        ...
        status = self._milvus.create_collection({'collection_name': self._table_name, 'dimension': X.shape[1], 'index_file_size': 2048})
        if not status.OK():
            raise Exception("Create collection {} failed: {}".format(self._table_name, status))
...
#             self._milvus_id_to_index[id] = i
    ...
    def query(self, v, n):
        ...
        v = v.tolist()
        status, results = self._milvus.search(collection_name=self._table_name, query_records=[v], top_k=n, params=self._search_param)
