...
class ONNG(BaseANN):
    ...
    def fit(self, X):
        ...
        index = os.path.join(index_dir, 'ONNG-' + str(self._edge_size) +
                             '-' + str(self._outdegree) + '-' + str(self._indegree))
        ...
        if (not os.path.exists(index)) and (not os.path.exists(anngIndex)):
            ...
            t = time.time()
            args = ['ngt', 'create', '-it', '-p8', '-b500', '-ga', '-of', '-D' + self._metric, '-d' + str(dim), '-E' + str(
                self._edge_size), '-S0', '-e' + str(self._epsilon), '-P0', '-B30', '-T' + str(self._build_time_limit), anngIndex]
        ...
        if not os.path.exists(index):
            ...
            t = time.time()
            args = ['ngt', 'reconstruct-graph', '-mS', '-o ' +
                    str(self._outdegree), '-i ' + str(self._indegree), anngIndex, index]
            subprocess.call(args)
    ...
    def set_query_arguments(self, epsilon):
        ...
        self.name = 'ONNG-NGT(%s, %s, %s, %s, %1.3f)' % (self._edge_size, self._outdegree,
                                                         self._indegree, self._edge_size_for_search, self._epsilon + 1.0)
