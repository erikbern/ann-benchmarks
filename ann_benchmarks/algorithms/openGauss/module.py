...
class openGaussHNSWPQ(BaseANN):
    def __init__(self, metric, method_param):
        ...
        self.con = 80
        self.subspace_dim = 1
        self.hnsw_earlystop_threshold = 2147483647
    ...
    def fit(self, X):
        ...
        if self._metric == "angular":
            cur.execute(
                "CREATE INDEX ON items USING hnsw (embedding vector_cosine_ops) WITH (m = %d, ef_construction = %d, enable_pq = on, pq_m = %d)" % (self._m, self._ef_construction, int(X.shape[1]/self.subspace_dim)))
        elif self._metric == "euclidean":
            cur.execute("CREATE INDEX ON items USING hnsw (embedding vector_l2_ops) WITH (m = %d, ef_construction = %d, enable_pq = on, pq_m = %d)" % (self._m, self._ef_construction, int(X.shape[1]/self.subspace_dim)))
        else:
            raise RuntimeError(f"unknown metric {self._metric}")
        print("done!")
        self._cur = cur
    ...
    def __str__(self):
        return f"openGaussHNSWPQ(m={self._m}, ef_construction={self._ef_construction}, ef_search={self._ef_search}, subspace_dim={self.subspace_dim}, hnsw_earlystop_threshold={self.hnsw_earlystop_threshold})"
