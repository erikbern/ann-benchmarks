/**
 * @file generate-random-inputs-unit-sphere.cpp 
...
vecType generateRandomVector(size_t d) {
    vecType v(d);
...
vecType distortPoint(const vecType& v, float targetDistance) {
    size_t d = v.size();
    vecType w(d); 
    for (size_t i = 0; i < d; i++)
...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; i < numClusters; i++) {
        querySet.insert(generateRandomVector(d));
    }
    ...
    for (auto& q: querySet) {
        ...
        for (size_t i = 0; i < numPoints; i++) {
            set.insert(distortPoint(q, targetDistance));
        }
        // Compute some noise.
        targetDistance *= 2;
    ...
    while (set.size() < n) {
        set.insert(generateRandomVector(d));
	}
